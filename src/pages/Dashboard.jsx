    ...
    try {
      ...
      const [resAtend, resEmpresas, resEstoque, resAmanha] = await Promise.all([
        supabase
          .from("atendimentos")
          .select(
            `id, data, cidade, tipo_servico, status, empresa_id, telefone,
        ...
        supabase
          .from("atendimentos")
          .select(
            `id, data, cidade, endereco, responsavel, telefone,
             empresas ( razao_social, nome_fantasia )`,
          )
                      ...
                      >
                        ...
                        <span
                          onClick={(e) => {
                            ...
                            if (a.telefone) {
                              window.open(
                                `https://wa.me/55${a.telefone.replace(/\D/g, "")}`,
                                "_blank",
                              );
                            }
                          }}
                          className={`shrink-0 p-1.5 rounded-full cursor-pointer transition-colors ${
                            a.telefone
                              ? "text-green-500 hover:bg-green-50 active:bg-green-100"
                              : "text-gray-300"
                          }`}
                          title={a.telefone ? "WhatsApp" : "Sem telefone"}
                        >
                          <svg
                            viewBox="0 0 24 24"
                            fill="currentColor"
                      ...
                      return (
                        ...
                        >
                          ...
                          <a
                            ...
                            target={a.telefone ? "_blank" : undefined}
                            rel="noopener noreferrer"
                            onClick={(e) => {
                              if (!a.telefone) e.preventDefault();
                            }}
